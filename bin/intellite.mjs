...
const APP_GUIDANCE_VERSION = "2026-07-10.1";
...
- Import \`intellite/intellite-proxy.mjs\` in every Intellite-exposed route and verify \`X-Intellite-Proxy-*\` ES256 signatures through the published JWKS.
...
See \`.intellite/examples/usage-guide.md\`, \`.intellite/examples/proxy-signature-verification.md\`, and \`intellite/intellite-proxy.mjs\`.
`
    },
    {
...
const actor = await verifyIntelliteProxyRequest(request, {
  appId: "${appId}",
...
async function projectSourceFiles(projectRoot, limit = 3000) {
  ...
  while (stack.length > 0 && files.length < limit) {
    ...
    for (const entry of entries) {
      ...
      if (relativePath === "intellite/intellite-proxy.mjs" || /(?:^|\/)[^/]+\.(?:test|spec)\.[^/]+$/i.test(relativePath)) continue;
...
async function implementationEvidence(projectRoot, manifest) {
  ...
  for (const file of files) {
    ...
    const hasVerifier = /verifyIntelliteProxyRequest\s*\(/.test(content) || (
    ...
    if (hasVerifier || hasUsageGuide || hasCapability || hasReplay) matched.add(file.path);
...
function adoptedAppManifest({ appId, name, localUrl, stagingUrl, productionUrl }) {
  return {
    ...
    environments: {
      local: { appBaseUrl: localUrl, proxyBaseUrl: localUrl },
      staging: { appBaseUrl: stagingUrl, proxyBaseUrl: stagingUrl },
      production: { appBaseUrl: productionUrl, proxyBaseUrl: productionUrl }
    },
    proxyRoutes: [{
      routeId: "usage-guide-read",
      method: "READ",
